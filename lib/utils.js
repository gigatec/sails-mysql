...
utils.buildSelectStatement = function(criteria, table, schemaDefs) {
  ...
  query += 'SELECT ' + (criteria.distinct ? 'DISTINCT ' : '');
  ...
  _( schemaDefs[table] ).forEach(function(schemaDef, key) {
    var keyDef = key.split(' AS ');
    var selectKey = keyDef[0].indexOf('.') !== -1 ? { key: keyDef[0], raw: true } : { table: table, key: key };
    if (keyDef.length == 2) selectKey.as = keyDef[1];
    selectKeys.push(selectKey);
  });
  if (criteria.select_sql) {
    criteria.select_sql.forEach(function(selectKey) {
      selectKeys.push(selectKey);
    });
  }
  ...
  selectKeys.forEach(function(select) {
    var quote = select.raw ? '' : '`';
    query += (select.table ? quote + select.table + quote + '.' : '') + quote + select.key + quote + (select.as ? ' AS ' + select.as : '') + ', ';
  });
