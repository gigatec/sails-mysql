...
var sql = module.exports = {
  ...
  prepareCriterion: function(collectionName, value, key, parentKey) {
    ...
    if (parentKey) {
      ...
      else if (key === 'notLike') return attrStr + ' NOT LIKE \'' + nakedButClean + '\'';
  ...
  buildSingleQuery: function(collectionName, options, tableDefs) {
    ...
    // Add join_sql feature
    if (options.join_sql) {
      queryPart += (Array.isArray(options.join_sql) ? options.join_sql.join(' ') : options.join_sql) + ' ';
    }

    // Add where_sql feature
    if (options.where || options.where_sql) {
      var where1 = options.where ? sql.where(collectionName, options.where) : '1';
      var where2 = options.where_sql ? options.where_sql : '1';
      if (Array.isArray(where2)) {
        where2 = options.where_sql.join(' AND ');
      }
      queryPart += 'WHERE (' + where1 + ') AND (' + where2 + ') ';
    }
...
function validSubAttrCriteria(c) {
  ...
  !_.isUndefined(c.startsWith) || !_.isUndefined(c.endsWith) || !_.isUndefined(c.contains) || !_.isUndefined(c.like) || !_.isUndefined(c.notLike));
}
